...
import './style.scss'
import { RiFilterLine, RiExternalLinkFill } from 'react-icons/ri'
import avatar1 from '../../../../assets/images/avatars/1.jpg'
import { useNavigate } from 'react-router-dom'
import Moment from 'moment';
import { Input } from '../../../widgets/formComponents/Input'
import { useParams } from 'react-router-dom'
import { useGetAllFinanceTicketByEmpId } from '../../../../query/financeHelpdesk/getHelpdesk/getHelpdeskQuery'
const Index = () => {
	const params = useParams();
	const navigate = useNavigate();
	const empDetail = useRecoilValue(authState)
	const [layout, setLayout] = useState('min');
	const { data: MyRequests, isLoading } = useGetAllFinanceTicketByEmpId(empDetail.uuid);
				...
				<Col className='col order-2 order-sm-2 order-md-1' xs={12} md={filterMode ? 9 : 12}>
					<div className='my-request'>
						<div className='cardHeader'>
							...
							<p className='cols dateCol'>DATE</p>
							...
							<>
								<Row>
									<Col sm={5} xs={12} md={12} className='filterRequest'>
										...
										<Input className='searchBar px-2 mb-3' type="text" placeholder="Search in all tickets..." />
...
const Card = ({ subject, date, name, empId, ticketNo, status, subcategory, priority, assignedTo, id }) => {
	const getColor = () => status == 'Open' ? 'primary' : status == 'Closed' ? 'secondary' : status == 'In Progress' ? 'warning' : 'danger'
	...
	return (
		...
		<>
			<Row className='myRequestCardElement' onClick={() => navigate(`/helpDesk/finance/request/${id}`, { state: { access: 'employee' } })}>
				<Col className='cols first-col'>
					...
					<div className='details' >
						<p>{subject}</p>
						<p >
							<b style={{ color: getHexColor() }}>Ticket Number: {ticketNo}</b> {empId} {assignedTo}
						</p>
					</div>
				</Col>
